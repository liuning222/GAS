...
class Net(torch.nn.Module):
    def __init__(self, args):
        ...
        self.layer1 = SCAN(self.num_features, self.nhid, ratio=self.ratio[0], heads=self.heads, mode=self.mode)

        self.layers = torch.nn.ModuleList()
        for i in range(args.num_layers - 1):
            self.layers.append(SCAN(self.nhid, self.nhid, ratio=self.ratio[i+1], heads=self.heads, mode=self.mode))
