...
def filter_dn_case2(edge_index, x_j, alpha, max_index, perm,num_nodes):
    mask = perm.new_full((num_nodes,), -1)
    i = torch.arange(perm.size(0), dtype=torch.long, device=perm.device)
    ...
    non_perm_index = torch.nonzero(torch.isin(source,non_perm)).squeeze()
    topk_max_index =torch.nonzero(~torch.isin(max_index, non_perm_index)).squeeze()
    topk_max = max_index[topk_max_index]

    source_mask = mask[source]
    source_mask[topk_max] = source[topk_max]
    ...
    return edge_index_mask, x_j_mask, alpha_mask
...
def filter_de_case3(edge_index, x_j, alpha, max_index, perm, num_nodes):
    ...
    source, target = edge_index
    source_non_perm = torch.nonzero(torch.isin(source, non_perm)).squeeze()
