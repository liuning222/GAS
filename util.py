...
def topk(x, ratio, batch, min_score=None, tol=1e-7):
    if min_score is not None:
        ...
        value = value + cum_num_nodes.view(-1, 1)
        value = value.view(-1)
...
def filter_de(edge_index, x_j, alpha, max_index, perm, num_nodes):
    mask = perm.new_full((num_nodes,), -1)
    i = torch.arange(perm.size(0), dtype=torch.long, device=perm.device)
    mask[perm] = i
...
def filter_new(out, edge_index_mask, edge_index, x_j, alpha, perm, max_index, num_nodes):
    mask = perm.new_full((num_nodes,), -1)
    i = torch.arange(perm.size(0), dtype=torch.long, device=perm.device)
    mask[perm] = i
    ...
    return out_final, edge_index_final, i
